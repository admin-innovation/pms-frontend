...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const dataLoad = async () => {
      const employeeData = await getEmployee(userId);
      setEmployeeData(employeeData);
      await getGoals(1000);
      await getNotifications(userId);

      if (employeeData && employeeData.department) {
        await getEmployees(employeeData.department);
        await getTasks(employeeData.department);
      }
    };
    ...
    <>
      <div className=" bg-[#F1F4F9]   flex flex-row  relative w-screen max-h-screen overflow-y-hidden overflow-x-hidden   py-[30px]   ">
        ...
        >
          ...
          >
            {/* <div className="w-full h-full absolute backdrop-blur-lg" /> */}
            <Header />
          </div>
